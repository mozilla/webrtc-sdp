...
impl StatefulSdpAnonymizer {
	...
	pub fn mask_cert_finger_print(&mut self, finger_print: &[u8]) -> Vec<u8> {
		...
		self.cert_finger_prints.insert(finger_print.to_vec(),
...
impl AnonymizationStrMap {
	...
	pub fn mask(&mut self, value: &str) -> String {
		...
		let store = format!("{}{:0padding$}", self.prefix, self.counter.0, padding = self.padding);
...
mod tests {
	...
	fn test_mask_ice_password() {
		...
		let masked_passwords = ["ice-password-00000001",
	...
	fn test_mask_ice_user() {
		...
		let masked_users = ["ice-user-00000001",
							"ice-user-00000002",
							"ice-user-00000003",];
		for _ in 0..2 {
	...
	fn test_mask_cert_fingerprint() {
		let mut anon = StatefulSdpAnonymizer::default();
		let prints:[Vec<u8>; 3] = [vec![0x59u8, 0x4A, 0x8B, 0x73, 0xA7, 0x73,
										0x53, 0x71, 0x88, 0xD7, 0x4D, 0x58,
										0x28, 0x0C, 0x79, 0x72, 0x31, 0x29,
						   				0x9B, 0x05, 0x37, 0xDD, 0x58, 0x43,
										0xC2, 0xD4, 0x85, 0xA2, 0xB3, 0x66,
										0x38, 0x7A,],
					  			   vec![0x30u8, 0xFF, 0x8E, 0x2B, 0xAC, 0x9D,
									    0xED, 0x70, 0x18, 0x10, 0x67, 0xC8,
										0xAE, 0x9E, 0x68, 0xF3, 0x86, 0x53,
						   				0x51, 0xB0, 0xAC, 0x31, 0xB7, 0xBE,
										0x6D, 0xCF, 0xA4, 0x2E, 0xD3, 0x6E,
										0xB4, 0x28,],
					  			   vec![0xDFu8, 0x2E, 0xAC, 0x8A, 0xFD, 0x0A,
									 	0x8E, 0x99, 0xBF, 0x5D, 0xE8, 0x3C,
										0xE7, 0xFA, 0xFB, 0x08, 0x3B, 0x3C,
						   				0x54, 0x1D, 0xD7, 0xD4, 0x05, 0x77,
										0xA0, 0x72, 0x9B, 0x14, 0x08, 0x6D,
										0x0F, 0x4C,]];
		let masked_prints = [1u64.to_be_bytes().to_vec(),
